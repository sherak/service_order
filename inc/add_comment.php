...
function add_comment($form_add_comment) {
	...
	if(!$conn->insert_data('comment', $data))  {
   		$form_add_comment->set_error('add_comment_btn', '<div class="alert alert-danger" role="alert">There\'s been an error. Try again.</div>');
   		header('Location: my_account.php');
	}

	if(!$form_add_comment->check_errors()) {
		$form_add_comment->set_success_msg('<div class="alert alert-success" role="alert">Comment successfully added.</div>');
	}
}