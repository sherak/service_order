...
if($c->insert_data('post', $data)) {
	$add_post_alert = 'Post successfully added!';
	header("Location: my_account.php?add_post_alert={$add_post_alert}");
} 
else {
	$add_post_alert .= "There's been an error! Try again!" . ' ' .  $content . ' ' . $datetime . ' ' . $user_id;
	header("Location: my_account.php?add_post_alert={$add_post_alert}");
}